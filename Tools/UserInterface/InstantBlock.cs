...
public static class InstantBlock
{
	...
	public static string[]? List(Area area, string[]? items, bool dropdown = false, bool multiselect = true, string[]? selected = null)
	{
		...
		var block = TryCache<List>("", area, out var cached, dropdown ? Span.Dropdown : Span.Column);
...
#region Backend
	...
	private static T TryCache<T>(string text, Area area, out bool wasCached, Span span = Span.Column, bool skipUpdate = false) where T : Block
	{
		var type = typeof(T);
		wasCached = true;
