...
public static class Asteroids
{
	...
	{
		var window = new Window { Title = "Pure - Asteroids Example" };
		var hardware = new Hardware(window.Handle) { Mouse = { CursorCurrent = Mouse.Cursor.Crosshair } };
		...
		while (window.KeepOpen())
		{
			...
			// shooting ====================================================
			if ((hardware.Keyboard.IsAnyJustPressed() || hardware.Mouse.IsAnyJustPressed()) && shotCooldown < 0)
			{
				shotCooldown = 5f;
