...
public static class Prompts
{
	public static Prompt Create(TilemapManager maps)
	{
		...
		var prompt = new Prompt { Element = inputBox, Message = $"Would you press{Environment.NewLine}on the green tick?" };
		...
		prompt.OnDisplay(buttons =>
		{
			if(prompt.IsOpened)
			{
				...
				maps[3].SetBox(prompt.Position, prompt.Size, bg, Tile.BOX_CORNER_ROUND, Tile.SHADE_OPAQUE, Color.Gray.ToDark());
			}
