...
public static class ImmediateGUI
{
	public static void Run()
	{
		...
		while (Window.KeepOpen())
		{
			...
			var checkbox = InstantBlock.Checkbox((0, 5), "Checkbox");
			if (checkbox != null)
				log = $"checkbox:\n{checkbox}";

			var sliderHor = InstantBlock.Slider((0, 7), 9);
			percent.Text((9, 7), 2);
			...
			var input = InstantBlock.InputBox((0, 9, 10, 1), "Inputbox");
			if (input != null)
				log = $"inputbox:\n{input}";

			var scrollHer = InstantBlock.Scroll((0, 11), 11);
			if (float.IsNaN(scrollHer) == false)
				log = $"horizontal scroll:\n{scrollHer:F1}";

			InstantBlock.Tooltip = InstantBlock.CurrentPrompt != null ? default : (tooltip, Pivot.Left);
			var stepper = InstantBlock.Stepper((0, 13), "Stepper", 0f, 1f, 999f, -999f);
			if (float.IsNaN(stepper) == false)
				log = $"stepper:\n{stepper:F1}";
			InstantBlock.Tooltip = default;

			var dropdown = InstantBlock.List((0, 16, 6, 5), animals, true);
			if (dropdown != null)
				log = $"dropdown list:\n{dropdown.ToString(", ")}";

			var palette = InstantBlock.Palette((20, 20));
			if (palette != null)
				log = $"palette:\n{new Color(palette ?? 0).ToBrush()}color";

			var fileViewer = InstantBlock.FileViewer((15, 4, 30, 10));
			if (fileViewer != null)
				log = $"fileViewer:\n{fileViewer.ToString(", ")}";

			var sliderVer = InstantBlock.Slider((9, 14), 9, vertical: true);
			if (float.IsNaN(sliderVer) == false)
				log = $"vertical slider:\n{sliderVer:F1}";

			InstantBlock.Tooltip = InstantBlock.CurrentPrompt != null ? default : (tooltip, Pivot.Left);
			var scrollVer = InstantBlock.Scroll((11, 14), 9, vertical: true);
			if (float.IsNaN(scrollVer) == false)
				log = $"vertical scroll:\n{scrollVer:F1}";
			InstantBlock.Tooltip = default;

			var pages = InstantBlock.Pages((2, 24));
			if (float.IsNaN(pages) == false)
				log = $"pages: \n{pages:F1}";

			var list = InstantBlock.List((40, 20, 6, 5), items);
			if (list != null)
				log = $"regular list:\n{list.ToString(", ")}";
