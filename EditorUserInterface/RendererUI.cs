...
public class RendererUI : UserInterface
{
	...
	protected override void OnUpdateInputBox(string key, InputBox inputBox)
	{
		...
		if (string.IsNullOrWhiteSpace(e.Text) && e.CursorIndex == 0)
			tilemaps[1].SetTextRectangle(e.Position, e.Size, e.Placeholder, Color.Gray.ToBright(), false);

	}
	protected override void OnUpdateSlider(string key, Slider slider)
	{
		var e = slider;
		tilemaps[1].SetBar(e.Position, Tile.BAR_BIG_EDGE, Tile.BAR_BIG_STRAIGHT, Color.Gray, e.Size.width);
	...
	protected override void OnUpdatePanel(string key, Panel panel)
	{
		...
		tilemaps[2].SetBorder(e.Position, e.Size, Tile.BORDER_GRID_CORNER, Tile.BORDER_GRID_STRAIGHT, Color.Gray);
		tilemaps[0].SetRectangle((e.Position.x + 1, e.Position.y), (e.Size.width - 2, 1), new(Tile.SHADE_OPAQUE, Color.Gray.ToDark(0.2f)));
		tilemaps[2].SetTextLine((e.Position.x + offset, e.Position.y), e.Text, Color.White, e.Size.width);
	}
	...
	protected override void OnUpdatePalette(string key, Palette palette)
	{
		...
		var alpha = e.Opacity;
		tilemaps[1].SetBar(alpha.Position, Tile.BAR_BIG_EDGE, Tile.BAR_BIG_STRAIGHT, Color.Gray, alpha.Size.width);
		...
		tilemaps[1].SetRectangle((x, y - 3), (e.Size.width, 3), new(Tile.SHADE_OPAQUE, e.SelectedColor));
	}
