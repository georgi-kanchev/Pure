...
public class Keyboard
{
	...
	/// </summary>
	public string KeyTyped { get; internal set; } = "";
	/// <summary>
	...
	public void SimulatePress(Key key)
	{
	...
	public string ToText(Key key, bool shift)
	{
		var i = key;
	...
	public bool IsJustPressedAndHeld(Key key)
	{
	...
	public void OnPress(Key key, Action method)
	{
		if (onPress.TryAdd(key, method) == false)
			onPress[key] += method;
	}
	public void OnRelease(Key key, Action method)
	{
		if (onRelease.TryAdd(key, method) == false)
			onRelease[key] += method;
	}
	public void OnPressAndHold(Key key, Action method)
	{
		if (onHold.TryAdd(key, method) == false)
			onHold[key] += method;
	}

	public void OnPressAny(Action<Key> method)
	{
		onPressAny += method;
	}
	public void OnReleaseAny(Action<Key> method)
	{
		onReleaseAny += method;
	}
	public void OnPressAndHoldAny(Action<Key> method)
	{
		onHoldAny += method;
	}
...
#region Backend
	...
	internal void OnPress(object? _, KeyEventArgs e)
	{
		...
		var symbol = ToText(key, IsPressed(Key.ShiftLeft) || IsPressed(Key.ShiftRight));
	...
	internal void OnRelease(object? _, KeyEventArgs e)
	{
		var key = (Key)e.Code;
		...
		if (key is Key.ShiftLeft or Key.ShiftRight && KeyTyped != string.Empty)
			foreach (var k in pressed)
				KeyTyped = KeyTyped.Replace(ToText(k, true), string.Empty);
	...
	internal void OnType(object? _, TextEventArgs e)
	{
