...
public static class Flow
{
	...
	public static IEnumerator WaitForMethod(IEnumerator method)
	{
		return WaitForSignal(method.GetType().FullName);
	}
	...
	public static IEnumerator EndMethod()
	{
		return new Rule(() => false) { destroy = true };
	}
	public static void EndMethod(IEnumerator method)
	{
		for (var i = 0; i < flows.Count; i++)
			if (flows[i].GetType().FullName == method.GetType().FullName)
			{
				flows.Remove(flows[i]);
				i--;
			}
