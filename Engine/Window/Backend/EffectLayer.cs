...
void main(void)
{{
	// no idea why it needs 1 pixel offset, probably because the vertexes need an offset of +1, +1
	//viewSize.x -= 1.0;
	//viewSize.y -= 1.01;
...
//
	...
	tileCoord.y = 1 - tileCoord.y;

	vec4 dataColor = texture2D(data, tileCoord + vec2(halfPixel.x, 0.0));
