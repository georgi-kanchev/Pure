...
public static class Program
{
	...
	private static void CreateInspector()
	{
		...
		create.OnDisplay(() => editor.MapsUi.SetButtonIcon(create, new(Tile.CURSOR_CROSSHAIR, Color.Gray), 1));

		var remove = new Button();
		remove.OnInteraction(Interaction.Trigger, LayersRemove);
		remove.OnUpdate(() => ShowWhenLayerSelected(remove));
		remove.OnDisplay(() => editor.MapsUi.SetButtonIcon(remove, new(Tile.PUNCTUATION_DASH, Color.Gray), 1));
		...
		paletteColor.Brightness.OnItemDisplay(btn => editor.MapsUi.SetPagesItem(paletteColor.Brightness, btn));
	...
	{
		layers.BringUp(layers.ItemsSelected);
	}
	private static void LayersDown()
	{
		layers.BringDown(layers.ItemsSelected);
	}
