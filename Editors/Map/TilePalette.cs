...
using static Pure.Engine.Hardware.Keyboard;
...
internal class TilePalette
{
	...
	public TilePalette(Editor editor)
	{
		...
		AppMouse.OnPress(Mouse.Button.Left, () =>
		{
			if (Program.menu.IsHidden == false ||
			    editor.Prompt.IsHidden == false ||
			    inspector is { IsHovered: false } ||
			    layerTiles.IsHovered(AppWindow, AppMouse.CursorPosition) == false)
				return;

			var pos = layerTiles.PositionFromPixel(AppWindow, AppMouse.CursorPosition);
			var (vx, vy) = map.View.Position;
			selectedPos = ((int)pos.x + vx, (int)pos.y + vy);
			selectedSz = (1, 1);
		});
		AppMouse.OnPress(Mouse.Button.Left, OnMousePressed);
		AppMouse.OnRelease(Mouse.Button.Left, OnMouseRelease);

		AppKeyboard.OnPress(Key.C, () =>
		{
			var ctrl = AppKeyboard.IsPressed(Key.ControlLeft) || AppKeyboard.IsPressed(Key.ControlRight);
		...
		AppKeyboard.OnPress(Key.Delete, () =>
		{
	...
	public void Update(Inspector inspector, TerrainPanel terrainPanel)
	{
		...
		var (mx, my) = layerTiles.PositionFromPixel(AppWindow, AppMouse.CursorPosition);
		...
		if (layerTiles.IsOverlapping((mx, my)))
	...
	public void TryDraw()
	{
		...
		if (IsPaintAllowed() && AppMouse.IsPressed(Mouse.Button.Left))
...
#region Backend
	...
	static TilePalette()
	{
		AppMouse.OnRelease(Mouse.Button.Left, () => clickSeed = (-10000, 10000).Random());
	}
	private void UpdateSelected()
	{
		if (layerTiles.IsHovered(AppWindow, AppMouse.CursorPosition) &&
		    AppMouse.IsPressed(Mouse.Button.Left) &&
		    prevMousePos != mousePos &&
	...
	private void OnMouseHold(Tile randomTile, TileMap? tilemap)
	{
		...
		var pos = (mx, my);
		var tool = inspector.tools.Current;
		var lmb = AppMouse.IsPressed(Mouse.Button.Left);
		var (szw, szh) = (end.x - start.x, end.y - start.y);
		var (offX, offY) = (1, 1);
