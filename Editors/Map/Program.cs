...
global using Pure.Engine.Hardware;
...
public static class Program
{
	...
	{
		AppWindow.SetIconFromTile(editor.LayerTilesUi, (Tile.ICON_SKY_STARS, Color.Red.ToDark()),
...
#region Backend
	...
	private static void CreateMenu()
	{
		...
		AppMouse.OnPress(Mouse.Button.Right, () =>
		{
			...
			var (mx, my) = editor.LayerTilesUi.PositionFromPixel(AppWindow, AppMouse.CursorPosition);
			menu.IsHidden = false;
	...
	private static byte[] Save()
	{
		try
		{
			...
			var bytes = editor.MapsEditor.ToDataAsBytes().Compress().ToList();
			...
			var layersBytes = layerNames.ToArray().ToDataAsBytes();
			bytes.AddRange(layersBytes);

			var generatorBytes = terrainPanel.generator.ToDataAsBytes();
