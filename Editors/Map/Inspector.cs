...
internal class Inspector : Panel
{
	...
	public Inspector(Editor editor, TilePalette tilePalette)
	{
		...
		OnDisplay += () =>
		{
			...
			var isHovered = tilePalette.layerTiles.IsHovered(AppWindow, AppMouse.CursorPosition);

			SetLine(14, 18, 23, 28);

			if (tools?.Current < 9 && isHovered && editor.Prompt.IsHidden)
			{
...
#region Backend
	private const int MIDDLE = (int)LayerMapsUi.Middle, FRONT = (int)LayerMapsUi.Front;
	...
	private Button[] AddPaletteScrolls()
	{
		...
		paletteScroll.OnInteraction(Interaction.Scroll, () => paletteScrollV.Slider.MoveHandle(AppMouse.ScrollDelta));
