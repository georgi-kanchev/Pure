global using static Pure.Editors.Base.Editor;
...
public static class Program
{
	...
	{
		AppWindow.SetIconFromTile(editor.LayerTilesUi, (Tile.SHAPE_SQUARE_BIG_HOLLOW, Color.Blue),
...
#region Backend
	...
	{
		get => editor.LayerTilesMap.IsHovered(AppWindow, AppMouse.CursorPosition) &&
	...
	{
		get => editor.LayerTilesMap.PositionFromPixel(AppWindow, AppMouse.CursorPosition);
	}
	private static (float x, float y) MousePosPrompt
	{
		get => layerTiles.PositionFromPixel(AppWindow, AppMouse.CursorPosition);
	}
	...
	static Program()
	{
		...
		const int MIDDLE = (int)LayerMapsUi.Middle;
		const int FRONT = (int)LayerMapsUi.Front;
		const int PROMPT_MIDDLE = (int)LayerMapsUi.PromptMiddle;
		const int PROMPT_BACK = (int)LayerMapsUi.PromptBack;
		tools = new((0, 0), 0)
		{
			Size = (10, 3),
			ItemSize = (10, 1),
	...
	private static void SubscribeToClicks()
	{
		...
		AppMouse.OnRelease(Mouse.Button.Left, () =>
		{
			...
			else if (CanDrawLayer && layerTiles.IsHovered(AppWindow, AppMouse.CursorPosition))
			{
		...
		AppMouse.OnPress(Mouse.Button.Right, () =>
		{
			if (CanEditLines)
			{
	...
	private static void CreateMenu()
	{
		...
		menu.OnItemInteraction(Interaction.Trigger, btn =>
		{
			...
			if (index == 1) // load tileset
				...
				AppWindow.Clipboard = Convert.ToBase64String(Save());
	...
	private static byte[] Save()
	{
		try
		{
			...
			if (selection == 0)
				data = solidPack.ToDataAsBytes();
			else if (selection == 1)
				data = solidMap.ToDataAsBytes();
			else if (selection == 2)
				data = linePack.ToDataAsBytes();
